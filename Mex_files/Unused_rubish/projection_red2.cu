...
__global__ void kernelPixelDetector( Geometry geo,
    ...
    __shared__ double sum[MAXTREADS]; // fits in the NVIDIA Tesla k40c, not sure if in tohers
    sum[tidx]=0;
    
    __syncthreads();
    for ( i = tidx; i < length; i += MAXTREADS ){
    ...
    // reduce sum. This reduces if MAXTREAD is 1024.
    // Modify if its not
    if ( tidx <512)
    ...
    __syncthreads();
    if ( tidx <32)
        sum[tidx] += sum[tidx + 32];
    __syncthreads();
    if ( tidx <16)
        sum[tidx] += sum[tidx + 16];
    __syncthreads();
    if ( tidx <8)
        sum[tidx] += sum[tidx + 8];
    __syncthreads();
    ...
    if (tidx<1)
    {
        sum[tidx] += sum[tidx + 1];
        __syncthreads();
        detector[idx]=sum[1]*deltalength;
    }
...
int projection(float const * const img, Geometry geo, double** result,double const * const alphas,int nalpha){
    ...
    for (int i=0;i<nalpha;i++){
        ...
        //Ray tracing!  
        kernelPixelDetector<<<geo.nDetecU*geo.nDetecV,MAXTREADS>>>(geo,dProjection, source, deltaU, deltaV, uvOrigin);
