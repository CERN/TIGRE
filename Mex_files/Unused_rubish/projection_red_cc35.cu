...
double warpReduceSum(double val) {
  for (int offset = MAXTHREADS/2; offset > 0; offset /= 2) 
...
double blockReduceSum(double val) {
  ...
  int lane = threadIdx.x % MAXTHREADS;
  int wid = threadIdx.x / MAXTHREADS;
  ...
  val = (threadIdx.x < blockDim.x / MAXTHREADS) ? shared[lane] : 0;
...
__global__ void kernelPixelDetector(Geometry geo,
    ...
    if ( idx >= geo.nDetecU * geo.nDetecV )
  	...
  	if ( tidx == 0 ) {
	    ...
	    //here comes the deal
	    deltalength = sqrt((vectX * geo.dVoxelX) * (vectX * geo.dVoxelX) +
    ...
    for ( i = tidx; i < length; i += MAXTHREADS ){
        x = vectX  * i + Sx;
        y = vectY  * i + Sy;
        z = vectZ  * i + Sz;
...
int projection(float const * const img, Geometry geo, double** result,double const * const alphas,int nalpha){
    ...
    for (int i=0;i<nalpha;i++){
        ...
        //Ray tracing!  
        kernelPixelDetector<<<geo.nDetecU*geo.nDetecV,MAXTHREADS>>>(geo,dProjection, source, deltaU, deltaV, uvOrigin);
