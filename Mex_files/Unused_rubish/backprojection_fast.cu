...
__global__ void kernelweigths(const Geometry geo,double* weigths, Point3D xyzOffset ){
    
     size_t idx = threadIdx.x + blockIdx.x * blockDim.x;
     if (idx>= geo.nVoxelX* geo.nVoxelY)
        return;
    int indY = idx / geo.nVoxelX;
    int indX = idx % geo.nVoxelX;
...
__global__ void kernelPixelBackprojection(const Geometry geo, 
    ...
    //Source
     ...
     image[idx]+=tex3D(tex, u +0.5 ,
                            v +0.5 , 
                            indAlpha                                           +0.5);
     image[idx]=weigths[indY*geo.nVoxelX+indX];
...
int backprojection(float const * const projections, Geometry geo, double* result,double const * const alphas,int nalpha){
    ...
    double* weigths;
    num_bytes = geo.nVoxelX*geo.nVoxelY* sizeof(double);
    cudaMalloc((void**)&weigths, num_bytes);
    ...
    for (int i=0;i<nalpha;i++){
        ...
        kernelweigths<<<(geo.nVoxelX*geo.nVoxelY+ MAXTREADS-1) / MAXTREADS,MAXTREADS>>>(geo,weigths,offOrig);
        
        kernelPixelBackprojection<<<(geo.nVoxelX*geo.nVoxelY*geo.nVoxelZ + MAXTREADS-1) / MAXTREADS,MAXTREADS>>>
                (geo,dimage,i,deltaX,deltaY,deltaZ,xyzOrigin,offOrig,offDetec,weigths);
