function [proj,geo,angles]=loadDiondoProjections(filepath,geo,angles,varargin)
...
if isempty(dir([filepath,'Projection']))
    error("Projection folder not found")
end
filepath=[filepath,'Projection/'];
%% get filename
...
for ii=1:length(angles_to_load)
    ...
    fid=fopen([filepath,filename,num2str(index(ii),'%04d'),'.raw'],'r');
    data=single(fread(fid,'uint16'));
    fclose(fid);
