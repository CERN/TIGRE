...
if strcmp(mode,'cone')
    ...
    geo.DSO = 1000;                             % Distance Source Origin        (mm)
    % Detector parameters
    geo.nDetector=[nDetector(1);1];					% number of pixels              (px)
    geo.sDetector=[512*0.8;1*0.8];            % total size of the detector    (mm)
    geo.dDetector=geo.sDetector./geo.nDetector;					% size of each pixel            (mm)

    % Image parameters
    geo.nVoxel=[nVoxel(1);nVoxel(2);1];                          % number of voxels              (vx)
...
addParameter(p,'mode','cone',@(x)(ischar(x)&&(strcmp(x,'parallel')||strcmp(x,'cone')||strcmp(x,'fan'))));
