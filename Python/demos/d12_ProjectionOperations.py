...
tigre.plotproj(np.concatenate([np.abs(projInterp3-projray), np.abs(projInterp1-projray), np.abs(projInterp05-projray), \
               np.abs(projInterp02-projray), np.abs(projInterp3005-projray), np.abs(projInterp3001-projray)],axis=1),angles,clims=[0, 20])
