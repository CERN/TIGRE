...
class initPICCS(IterativeReconAlg):  
    """
    ASD_POCS solves the ASD_POCS total variation constrained image in 3D
    tomography
    ...
    def __init__(self, proj, geo, angles, niter, **kwargs):
        ...
        self.prior_ratio = 0.2 if "prior_ratio" not in kwargs else kwargs["prior_ratio"]
    ...
    def run_main_iter(self):
        ...
        while not stop_criteria:
            ...
            res_prev = copy.deepcopy(self.res)
            self.res = getattr(self, self.regularization)(self.res, dtvg, self.prior, self.prior_ratio)
            dg_vec = self.res - res_prev
            dg = im3DNORM(dg_vec, 2)
...
class PICCS(initPICCS):
    __doc__ = initPICCS.__doc__
    ...
    def __init__(self, proj, geo, angles, niter, **kwargs):
        ...
        if "blocksize" in kwargs and kwargs['blocksize']>1:
            print('Warning: blocksize is set to 1, please use an OS version of the algorithm for blocksize > 1')
        kwargs.update(dict(blocksize=1))
        kwargs.update(dict(regularization="minimizeTV_prior"))
        
        initPICCS.__init__(self, proj, geo, angles, niter, **kwargs)
       

piccs = decorator(PICCS, name="piccs")

class OS_PICCS(initPICCS):
...
os_piccs = decorator(OS_PICCS, name="os_piccs")
